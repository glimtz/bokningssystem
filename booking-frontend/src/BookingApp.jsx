...
const initialState = {
  ...
  addons: { boat: { selected: false, days: 0 }, guide: { selected: false, days: 0, selectedDates: [] }, linens: { selected: false }, cleaning: { selected: false } },
  guests: { count: 2, name: '', email: '', phone: '', message: '' },
};
...
const bookingReducer = (state, action) => {
  switch (action.type) {
    ...
    case 'SET_DATES': return { ...state, dates: action.payload, addons: { ...state.addons, boat: { ...state.addons.boat, days: 0 }, guide: { ...state.addons.guide, days: 0, selectedDates: [] } } };
    case 'TOGGLE_ADDON': {
    ...
    case 'SET_ADDON_DAYS': {
      ...
      if (addonId === 'guide') update.selectedDates = [];
      return { ...state, addons: { ...state.addons, [addonId]: update } };
    }
    case 'SET_GUIDE_DATES': return { ...state, addons: { ...state.addons, guide: { ...state.addons.guide, selectedDates: action.payload } } };
...
const StepAddons = () => {
  ...
  const toggleGuideDate = (dateStr) => {
    const current = state.addons.guide.selectedDates, maxDays = state.addons.guide.days;
    if (current.includes(dateStr)) { dispatch({ type: 'SET_GUIDE_DATES', payload: current.filter((d) => d !== dateStr) }); } else if (current.length < maxDays) { dispatch({ type: 'SET_GUIDE_DATES', payload: [...current, dateStr] }); }
  };

  const stayDates = getStayDates(), guideAddon = state.addons.guide, showGuideDatePicker = guideAddon.selected && guideAddon.days > 0 && guideAddon.days < nightCount;
            ...
            <div key={addon.id} className="addon-card-wrapper">
              ...
              {addon.id === 'guide' && showGuideDatePicker && (
                <div className="guide-date-picker">
                  <label className="addon-day-label">{t('addons.selectWhichDays')} ({guideAddon.selectedDates.length}/{guideAddon.days})</label>
                  <div className="guide-date-grid">
                    {stayDates.map((date) => {
                      const dateStr = toDateString(date), isChosen = guideAddon.selectedDates.includes(dateStr), isFull = guideAddon.selectedDates.length >= guideAddon.days && !isChosen;
                      return <button key={dateStr} className={`guide-date-btn ${isChosen ? 'chosen' : ''} ${isFull ? 'disabled' : ''}`} onClick={() => toggleGuideDate(dateStr)} disabled={isFull}>{formatDateShort(date)}</button>;
                    })}
                  </div>
                </div>
              )}
...
const StepSummary = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const bookingData = {
        lodge: lodge.name, checkIn: state.dates.checkIn?.toISOString(), checkOut: state.dates.checkOut?.toISOString(), nights: nightCount, guests: guestCount,
        contact: { name: state.guests.name, email: state.guests.email, phone: state.guests.phone, message: state.guests.message },
        addons: selectedAddons.map((addon) => ({
          id: addon.id, name: addon.name, type: addon.type,
          ...
          ...(addon.id === 'guide' && state.addons.guide.selectedDates.length > 0 ? { selectedDates: state.addons.guide.selectedDates } : {}),
        })),
        lodgePricePerNight, lodgeTotal: lodgePricePerNight * nightCount, totalEstimate,
      };
          ...
          {selectedAddons.map((addon) => {
            ...
            return (
              <div key={addon.id}>
                ...
                {addon.id === 'guide' && state.addons.guide.selectedDates.length > 0 && <div className="summary-row" style={{ color: 'var(--fm-text-muted)', fontSize: '0.85rem' }}><span>{t('addons.guideDays')}: {state.addons.guide.selectedDates.map(formatDateShort).join(', ')}</span></div>}
              </div>
            );
          })}
        </div>
            ...
            <div className="language-switcher">
              {['en', 'sv', 'de', 'fr'].map((l) => (
                <button key={l} className={`lang-btn ${lang === l ? 'active' : ''}`} onClick={() => setLang(l)}>
                  <span className="lang-flag">{l === 'en' ? '🇬🇧' : l === 'sv' ? '🇸🇪' : l === 'de' ? '🇩🇪' : '🇫🇷'}</span>
